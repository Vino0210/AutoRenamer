...
def _get_date_prefix(p: Path, date_source: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        if date_source == 'exif':
            ...
            try:
                ts = p.stat().st_mtime
                return datetime.fromtimestamp(ts).strftime('%Y%m%d'), (note_code or 'exif_missing')
            except Exception:
                return None, (note_code or 'exif_missing')
    ...
    def _precheck_worker(self, token: int, target_path: str, is_single_file: bool, opts: RenameOptions):
        ...
        try:
            ...
            for p in files:
                ...
                date_prefix, _note_code = _get_date_prefix(p, opts.date_source)
                if not date_prefix:
                    continue
        ...
        try:
            ...
            for idx, file_path in enumerate(files, start=1):
                ...
                try:
                    ...
                    date_prefix, note_code = _get_date_prefix(file_path, opts.date_source)
                    if not date_prefix:
                        self._q_put({'type': 'log', 'tag': 'err', 'msg': t['error'].format(str(file_path), 'stat() failed')})
                        self._q_put({'type': 'progress', 'current': idx, 'total': result.total})
                        continue
                    ...
                    if opts.dry_run:
                        result.renamed += 1
                        self._q_put({'type': 'log', 'tag': 'preview', 'msg': t['preview_rename'].format(original_name, final_name) + (f" ({t['summary_exif_fallback']})" if note_code else '')})
                    else:
                        file_path.rename(final_path)
                        ops.append({'old': str(file_path), 'new': str(final_path)})
                        result.renamed += 1
                        self._q_put({'type': 'log', 'tag': 'success', 'msg': t['success_rename'].format(original_name, final_name) + (f" ({t['summary_exif_fallback']})" if note_code else '')})
