...
TEXTS = {
    'zh': {
        'title': '文件自动重命名',
        'subtitle': '根据文件修改时间添加日期前缀（YYYYMMDD_）',
...
class RoundedFrame(tk.Frame):
    ...
    - Canvas 绘制：圆角背景 + 边框 + 轻微阴影
    - inner_frame 承载内容（真正放控件的地方）
    """
    ...
    def _rounded_rect(
        ...
        *,
        ...
        self.canvas.create_arc(x1, y1, x1 + 2 * r, y1 + 2 * r,
                               start=90, extent=90, fill=fill, outline=outline, width=width, tags=tags)
        self.canvas.create_arc(x2 - 2 * r, y1, x2, y1 + 2 * r,
                               start=0, extent=90, fill=fill, outline=outline, width=width, tags=tags)
        self.canvas.create_arc(x2 - 2 * r, y2 - 2 * r, x2, y2,
                               start=270, extent=90, fill=fill, outline=outline, width=width, tags=tags)
        self.canvas.create_arc(x1, y2 - 2 * r, x1 + 2 * r, y2,
                               start=180, extent=90, fill=fill, outline=outline, width=width, tags=tags)

        # body
        self.canvas.create_rectangle(x1 + r, y1, x2 - r, y2, fill=fill, outline=outline, width=width, tags=tags)
        self.canvas.create_rectangle(x1, y1 + r, x2, y2 - r, fill=fill, outline=outline, width=width, tags=tags)
    ...
    def _on_canvas_configure(self, _event=None):
        ...
        self.canvas.itemconfigure(self._win_id, width=max(1, w - inset * 2), height=max(1, h - inset * 2))

        self._draw_card(w, h)

    def _draw_card(self, w: int, h: int):
        self.canvas.delete('card')
        self.canvas.delete('shadow')

        # 外边界留一点空间，避免圆角被裁切（也给阴影留空间）
        pad = 12
        x1, y1 = pad, pad
        x2, y2 = w - pad, h - pad

        # 高级柔化阴影：用 3 层不同“强度/偏移”模拟 CSS box-shadow 的扩散效果
        if self.shadow:
            layers = [
                (8, COLORS['shadow_3']),
                (5, COLORS['shadow_2']),
            ...
            for off, col in layers:
                self._rounded_rect(
                    x1 + off, y1 + off, x2 + off, y2 + off, self.radius,
                    fill=col, outline='', width=0, tags='shadow'
                )
        # 主体卡片
        self._rounded_rect(x1, y1, x2, y2, self.radius,
                           fill=self.bg_color, outline=self.border_color, width=1, tags='card')


class PillButton(tk.Canvas):
    """胶囊按钮（Canvas 自绘圆角 + hover）

    设计目标：
    - 实心填充的“胶囊”按钮（更接近 Dashboard / Apple 风格）
    - 无内部拼接线（旧实现用 arc+rectangle 且都带 outline，容易出现十字分割线）
    - 支持 .config(text=..., state=..., font=..., fill=..., outline=...) 以兼容现有代码
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 按压反馈：ButtonPress/Release（更接近原生 App 的“触感”）
    ...
    def _draw_round_fill(self, x1: int, y1: int, x2: int, y2: int, r: int, *, fill: str):
        # 纯填充：所有图形 outline=''，避免内部拼接线
    ...
    def _draw_round_outline(self, x1: int, y1: int, x2: int, y2: int, r: int, *, outline: str, width: int):
        if not outline or width <= 0:
            return

        # 线条：用 arc(style=ARC) + line 画一圈外边框，避免内部重叠线
        cap = tk.ROUND
        join = tk.ROUND
        ...
        self.create_line(x1 + r, y1, x2 - r, y1, fill=outline, width=width, capstyle=cap, joinstyle=join)
        self.create_line(x2, y1 + r, x2, y2 - r, fill=outline, width=width, capstyle=cap, joinstyle=join)
        self.create_line(x2 - r, y2, x1 + r, y2, fill=outline, width=width, capstyle=cap, joinstyle=join)
        self.create_line(x1, y2 - r, x1, y1 + r, fill=outline, width=width, capstyle=cap, joinstyle=join)
    ...
    def _redraw(self):
        ...
        # hover/press fill
        base_fill = self._fill_hover if (self._hover and self._state != tk.DISABLED) else self._fill
        fill = self._shade(base_fill, 0.92) if (self._pressed and self._state != tk.DISABLED) else base_fill

        # disabled: 轻微去饱和（用更浅的填充 + 灰字）
        if self._state == tk.DISABLED:
            # 让 disabled 看起来更像“不可点”，但仍保持卡片风
            fill = COLORS['bg_drop'] if fill != COLORS['bg_card'] else COLORS['bg_card']

        # shadow（很轻的右下偏移）
        if self._shadow:
            s = 2
            self._draw_round_fill(1 + s, 2 + s, w - 1 + s, h - 1 + s, r, fill=COLORS['shadow_2'])

        # main

        self._draw_round_fill(1, 1, w - 1, h - 1, r, fill=fill)
        self._draw_round_outline(1, 1, w - 1, h - 1, r, outline=self._outline, width=self._outline_width)
    ...
    def _on_release(self, e=None):
        ...
        w = self.winfo_width()
        h = self.winfo_height()
        if 0 <= e.x <= w and 0 <= e.y <= h and callable(self._command):
            self._command()
...
class RenameApp(TkinterDnD.Tk):
    ...
    def _init_ttk_style(self):
        ...
        try:
            ...
            style.configure('TCheckbutton', background=COLORS['bg_main'], font=self._font(12))
            style.configure('Card.TCheckbutton', background=COLORS['bg_card'], font=self._font(12))
    ...
    def _update_texts(self):
        ...
        self.title(t['title'])
        self.title_label.config(text=t['title'], font=self._font(30, 'bold'))
        self.subtitle_label.config(text=t['subtitle'], font=self._font(13))

        self.drop_area.config(text=t['drop_area'], font=self._font(15))
        ...
        self.filters_title.config(text=t['filters'], font=self._font(12, 'bold'))
...
# ---- Target card ----
        ...
        inner = tk.Frame(card_drop.inner_frame, bg=COLORS['bg_card'], padx=16, pady=16)
        ...
        preview_card = RoundedFrame(right, radius=16)
        preview_card.grid(row=0, column=0, sticky='nsew')
        prev = tk.Frame(preview_card.inner_frame, bg=COLORS['bg_card'], padx=14, pady=14)
        ...
        detail.tag_config('title', font=self._font(11, 'bold'), foreground=COLORS['text_primary'])
        detail.tag_config('muted', font=self._font(11), foreground=COLORS['text_secondary'])
        ...
        log_card = RoundedFrame(right, radius=16)
        log_card.grid(row=1, column=0, sticky='nsew', pady=(12, 0))
        log_inner = tk.Frame(log_card.inner_frame, bg=COLORS['bg_card'], padx=16, pady=14)
