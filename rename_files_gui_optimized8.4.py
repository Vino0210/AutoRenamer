...
class RoundedFrame(tk.Frame):
    ...
    def _draw_rounded_rect(
        ...
        *,
        ...
        self.canvas.create_polygon(
            ...
            fill=fill,
            outline=outline,
            width=(width if outline and width > 0 else 0),
            tags=tags,
        )
    ...
    def _draw_bg(self, w: int, h: int, m: int):
        ...
        # 柔化阴影：不同偏移 + 不同浅色模拟扩散
        if self.shadow:
            self._draw_rounded_rect(m + 2, m + 6, w - m + 2, h - m + 6, r, fill=COLORS['shadow_1'], outline='', width=0)
            self._draw_rounded_rect(m + 1, m + 4, w - m + 1, h - m + 4, r, fill=COLORS['shadow_2'], outline='', width=0)
            self._draw_rounded_rect(m + 0, m + 3, w - m + 0, h - m + 3, r, fill=COLORS['shadow_3'], outline='', width=0)
