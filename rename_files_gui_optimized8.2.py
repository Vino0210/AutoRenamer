...
class RoundedFrame(tk.Frame):
    ...
    def _draw_card(self, w: int, h: int):
        ...
        if self.shadow:
            s = 2
            self._rounded_rect(x1 + s, y1 + s, x2 + s, y2 + s, self.radius,
                               fill=COLORS.get('shadow', '#E6E6EE'), outline='', width=0, tags='shadow')
...
class PillButton(tk.Canvas):
    ...
    def _redraw(self):
        ...
        # hover fill
        fill = self._fill_hover if (self._hover and self._state != tk.DISABLED) else self._fill
        ...
        if self._shadow:
            s = 2
            self._draw_round_fill(1 + s, 1 + s, w - 1 + s, h - 1 + s, r, fill=COLORS.get('shadow', '#E6E6EE'))
